...
class Newmofang(object):
    ...
    def __init__(self,**kwargs):
        ...
        self.search_dict = {'platform_kpi':'vv_cid_day','platform_vv_day': 'vv_cid_day', 'platform_uv_day': 'bid_uv_day','platform_pv_day':'dau_bid_day',
                            ...
                            'channel_uv_pid_day_avg': 'pid_uv_day', 'channel_vv_pid_day_avg': 'vv_pid_day'
                            }
        ###查询对应模块所用类内部函数
        self.func_dict = {"platform_kpi":self.platform_kpi,"platform_vv_month": self.platform_vv_month,"platform_vv_day": self.platform_vv_day,"platform_pv_day":self.platform_pv_day,"platform_uv_day": self.platform_uv_day,
                          ...
                          "channel_uv_pid_day_avg":self.channel_uv_pid_day_avg,"channel_vv_pid_day_avg":self.channel_vv_pid_day_avg,
                          }
        ###魔方数据库中bid对应的终端
    ...
    def channel_vv_pid_day_avg(self):
        sql = "select pid,round(sum(vv)/7) as vv from {table} where {bid_not_in} and cid in ({channel_cid}) and pid in ({pid_str}) and date>='{start_time}' and date<='{end_time}' group by pid order by round(sum(vv)/7) desc limit 20".format(table=self.table,pid_str=self.pid_str,start_time=self.start_date,end_time=self.end_date,bid_not_in=self.bid_not_in,channel_cid=self.channel_cid)
        return sql
    ...
    def channel_uv_pid_day_avg(self):
        sql = "select pid,round(sum(uv)/7) as uv from {table} where {bid_not_in} and cid in ({channel_cid}) and date>='{start_time}' and date<='{end_time}' group by pid order by round(sum(uv)/7) desc limit 20".format(table=self.table,start_time=self.start_date,end_time=self.end_date,bid_not_in=self.bid_not_in,channel_cid=self.channel_cid)
        return sql
...
if __name__ == '__main__':
    ...
    print Newmofang(searchtype='platform_vv_pid_day_avg',channel_name='show',start_date='20170301',end_date='20170307').sql_query()
